...
const claw = {
    ...
    offsetForward: 50,  // how far ahead/above the boat to start extending
...
function update() {
    ...
    const clawBaseX = boat.x + Math.sin(boat.angle) * claw.offsetForward - Math.cos(boat.angle) * claw.offsetSide;
    const clawBaseY = boat.y - Math.cos(boat.angle) * claw.offsetForward - Math.sin(boat.angle) * claw.offsetSide;
...
function draw() {
    ...
    if (claw.length > 0) {
        ...
        ctx.fillRect(claw.x-8, claw.y-8, 16, 16);
    }
